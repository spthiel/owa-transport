...
export default class HtmlParser {
    constructor(html, templateName) {
        ...
        if (this.fragment.childElementCount > 1) {
            throw new Error(`Template ${templateName} has too many root elements (${this.fragment.childElementCount})`);
        }
    }
    ...
    parse() {
        ...
        return `
export default function(): {${Object.entries(this.exports).map(([name, type]) => `${name}: ${type}`).join(", ")}} {
    ...
    _step(node) {
        ...
        if (node.hasAttribute('ref')) {
            ...
            if (varName in this.exports) {
                throw new Error(`Template ${this.templateName} has multiple elements with the ref ${varName}`);
            }
            node.removeAttribute('ref');
            this.exports[varName] = typeNames[node.tagName.toLowerCase()] || "HTMLElement";
        } else {
        ...
        for (const child of node.childNodes) {
            const childVarName = child.nodeType === 1 ? this._step(child) : this._stepTextNode(child);
            if (childVarName) {
